export default function Navbar({value,setvalue}){
    return(
        ...
        <div className="flex justify-between w-full h-fit p-5 bg-linear-90 from-red-200 via-red-50 to-lime-300 shadow-2xl shadow-red-400">
            ...
            <button onClick={()=>{
                setvalue(!value) }} className="h-fit w-fit p-4 border-2 font-bold bg-linear-90 from-red-200 via-lime-200 to-red-200 text-white hover:text-red-300 hover:shadow-2xl hover:shadow-red-400">{value? "Hide Form":"Show Form"}</button>
        </div>
        </>
    )
}